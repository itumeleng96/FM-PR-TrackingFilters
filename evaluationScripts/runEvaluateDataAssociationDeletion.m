...
rangeTrueData1 = h5read('./groundTruthCalculations/true_data1.h5', '/bistatic_ranges');
dopplerTrueData1 = h5read('./groundTruthCalculations/true_data1.h5', '/doppler_shifts');

rangeTrueData2 = h5read('./groundTruthCalculations/true_data2.h5', '/bistatic_ranges');
dopplerTrueData2 = h5read('./groundTruthCalculations/true_data2.h5', '/doppler_shifts');

trueData2 = [rangeTrueData2;dopplerTrueData2];
...
for pfa_index = 1:length(pfa_values)
    ...
    totalDeletedKalman = 0;
    totalDeletedUKF = 0;
    totalDeletedParticle = 0;
    totalDeletedRGNF = 0;
    ...
    for j = 1:numSimulations
        ...
        for i = 1:simulation_time
            ...
            [targetClusters, RDM, rdm_] = ca_cfar(y.', pfa, fs, dopp_bins, delay, 25);
        ...
        totalDeletedKalman = totalDeletedKalman + compareTracks(multiTargetTracker1.getDeletedTracks, {trueData, trueData1,trueData2}, 0.5, 1, 10, 0.01);
        totalDeletedUKF = totalDeletedUKF + compareTracks(multiTargetTracker2.getDeletedTracks, {trueData, trueData1,trueData2}, 0.5, 1, 10, 0.01);
        totalDeletedParticle = totalDeletedParticle + compareTracks(multiTargetTracker3.getDeletedTracks, {trueData, trueData1,trueData2}, 0.5, 1, 10, 0.01);
        totalDeletedRGNF = totalDeletedRGNF + compareTracks(multiTargetTracker4.getDeletedTracks, {trueData, trueData1,trueData2}, 0.5, 1, 10, 0.01);
    end

    % Calculate averages for the current PFA value
    avgKalmanResults(pfa_index) = totalDeletedKalman / numSimulations;
    avgUKFResults(pfa_index) = totalDeletedUKF / numSimulations;
    avgParticleResults(pfa_index) = totalDeletedParticle / numSimulations;
    avgRGNFResults(pfa_index) = totalDeletedRGNF / numSimulations;
end

% Display results
for pfa_index = 1:length(pfa_values)
    fprintf('\nResults for PFA = %.1e:\n', pfa_values(pfa_index));
    fprintf('  Average Confirmed Kalman Tracks: %.2f\n', avgKalmanResults(pfa_index));
    fprintf('  Average Confirmed UKF Tracks: %.2f\n', avgUKFResults(pfa_index));
    fprintf('  Average Confirmed Particle Tracks: %.2f\n', avgParticleResults(pfa_index));
    fprintf('  Average Confirmed RGNF Tracks: %.2f\n', avgRGNFResults(pfa_index));
end

function numDeletedTrueTracks = compareTracks(deletedTracks, trueTracks, rangeWeight, dopplerWeight, timeWindow, costOfNonAssignment)
    % Inputs:
    % - deletedTracks: Cell array of deleted tracks (each 2 x N array)
    ...
    % Number of confirmed true tracks is the number of valid assignments
    numDeletedTrueTracks = size(assignments, 1);
end
