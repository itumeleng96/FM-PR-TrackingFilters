...
% Initialize the full interpolation arrays
x_interp = [];
y_interp = [];
z_interp = [];
...
for i = 1:length(TargetWayPoints) - 1
    % Define cubic splines with at least 3 waypoints when possible
    if i < length(TargetWayPoints) - 1
        % Use 3 waypoints
        pp_x = spline(TargetWayPoints(i:i+2), TargetPos(1, i:i+2));
        pp_y = spline(TargetWayPoints(i:i+2), TargetPos(2, i:i+2));
        pp_z = spline(TargetWayPoints(i:i+2), TargetPos(3, i:i+2));
    else
        % For the last segment, use 2 waypoints
        pp_x = spline(TargetWayPoints(i:i+1), TargetPos(1, i:i+1));
        pp_y = spline(TargetWayPoints(i:i+1), TargetPos(2, i:i+1));
        pp_z = spline(TargetWayPoints(i:i+1), TargetPos(3, i:i+1));
    end
    
    % Get the unique segment time ranges
    if i < length(TargetWayPoints) - 1
    ...
    % Interpolate within the unique time segment
    x_interp_segment = ppval(pp_x, t_segment);
    y_interp_segment = ppval(pp_y, t_segment);
    z_interp_segment = ppval(pp_z, t_segment);
