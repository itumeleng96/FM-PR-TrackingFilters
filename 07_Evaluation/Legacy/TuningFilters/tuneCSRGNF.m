...
% Define the search space for optimization (including lambda)
optimVars = [
    optimizableVariable('std_acc1', [0.0001, 1]),   % Process noise std (for range)
    optimizableVariable('std_acc2', [0.0001, 1]),   % Process noise std (for doppler)
    optimizableVariable('std_meas1', [1, 5]),        % Measurement noise std (for range)
    optimizableVariable('std_meas2', [0.1, 2]),       % Measurement noise std (for doppler)
    optimizableVariable('lambda', [0.8, 1])         % Lambda parameter for tuning
];

% Objective function for Bayesian Optimization
objFun = @(params) rgnf_nees_loss(params.std_acc1, params.std_acc2, params.std_meas1, params.std_meas2, params.lambda, rangeMeasurementData, dopplerMeasurementData, groundTruthData);
...
optimal_params = [results.XAtMinObjective.std_acc1, ...
                  ...
                  results.XAtMinObjective.lambda];

disp('Optimized std_acc:');
disp(optimal_params(1:2));
disp('Optimized std_meas:');
disp(optimal_params(3:4));
disp('Optimized lambda:');
disp(optimal_params(5));

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Function Definitions (must be at the end in MATLAB)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function loss = rgnf_nees_loss(std_acc1, std_acc2, std_meas1, std_meas2, lambda, rangeMeasurementData, dopplerMeasurementData, groundTruthData)
    ...
    dt = 1;
    RGNF_object = CSRGNF(dt, std_acc, std_meas(1), std_meas(2), [x_initial(1); 0; x_initial(2); 0], 100, lambda);
    ...
    for i = 1:58
        % Prediction
        [X_pred, RGNF_object] = RGNF_object.predict();
        
        % Update stage
        [X_est, RGNF_object] = RGNF_object.update([rangeMeasurementData(i); dopplerMeasurementData(i)]);
        
        % Get the estimated state covariance (P) and estimation error
        P_k = RGNF_object.P;  % State covariance matrix after the update
