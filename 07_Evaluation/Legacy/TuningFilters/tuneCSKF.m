...
groundTruthData = [rangeTrueData; dopplerTrueData];
...
optimVars = [
    optimizableVariable('std_acc1', [0.0001, 0.5]),   % Process noise std (for range)
    optimizableVariable('std_acc2', [0.0001, 0.5]),   % Process noise std (for doppler)
...
objFun = @(params) kalman_nees_loss(params.std_acc1, params.std_acc2, params.std_meas1, params.std_meas2, rangeMeasurementData, dopplerMeasurementData, groundTruthData);
...
function loss = kalman_nees_loss(std_acc1, std_acc2, std_meas1, std_meas2, rangeMeasurementData, dopplerMeasurementData, groundTruthData)
    ...
    dt = 1;
    KF_object = CSKF(dt, std_acc, std_meas(1), std_meas(2), [x_initial(1);0;x_initial(2);0;]);
    ...
    for i = 1:58
        % Prediction
        [X_pred, KF_object] = KF_object.predict();
        
        % Update stage
        [X_est, KF_object] = KF_object.update([rangeMeasurementData(i); dopplerMeasurementData(i)]);
        
        % Get the estimated state covariance (P) and estimation error
        P_k = KF_object.P;  % State covariance matrix after the update
