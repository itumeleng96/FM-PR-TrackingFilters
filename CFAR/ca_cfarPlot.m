...
figure(10);
clf;
plot(frequency_levels, specific_range_line_rdm, 'LineWidth', 2);
hold on;
plot(frequency_levels, specific_range_line_threshold, 'LineWidth', 2);
% Set the axis labels and title
xlabel('Frequency');
ylabel('Signal Levels(DB)');
title('CFAR validation for Range:10KM');
...
text(0, 10, "Time: " + index + "s", 'Color', 'white'); 
axis xy;
xlabel('Bistatic range [m]', 'FontSize', 10, 'Color', 'black');
ylabel('Bistatic Doppler frequency [Hz]', 'FontSize', 10, 'Color', 'black');
grid on;
title('CFAR and Centroids', 'Color', 'black'); 
