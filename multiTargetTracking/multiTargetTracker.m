classdef multiTargetTracker
    ...
    methods
        ...
        function obj = plotMultiTargetTracking(obj,fs,fd_max,td_max,index,f,RDM)
                ...
                for i = 1:length(obj.tracks)
                    if(obj.tracks(i).deleted ==0)
                        ...
                        if obj.tracks(i).confirmed == 0
                            ...
                            plot(obj.tracks(i).trueTrack(1,:), obj.tracks(i).trueTrack(2,:), '-', 'LineWidth', 2, 'Color','green' ,'MarkerSize', 4, 'DisplayName', 'Confirmed(Measurement) Track');
                        else
                            plot(obj.tracks(i).predictedTrack(1, :), obj.tracks(i).predictedTrack(2, :), '^', 'MarkerFaceColor', 'none','MarkerEdgeColor', 'blue', 'MarkerSize', 6, 'DisplayName', 'Predicted Track');                        % Plot true track as open circles joined by a line
                ...
                legend([predicted_marker, tentative_marker, confirmed_marker], 'Prediction', 'Tentative', 'Confirmed(Measurement) Track', 'Location', 'best');
                %hold on;
        end
        ...
        function obj = plotMultiTargetTrackingId(obj, fs, fd_max, td_max, index, f, RDM)
                ...
                if track.deleted == 0  % Only plot active tracks
                    ...
                    if track.confirmed == 0
                        ...
                        plot(track.predictedTrack(1, :), track.predictedTrack(2, :), '^', ...
                            ...
                            'MarkerSize', 6, 'DisplayName', 'Predicted Track');
                        plot(track.trueTrack(1, :), track.trueTrack(2, :), '-', ...
                            'LineWidth', 2, 'Color', 'red', 'DisplayName', 'Confirmed(Measurement) Track');
                    end
            ...
            legend([predicted_marker, tentative_marker, confirmed_marker], ...
                   'Prediction', 'Tentative Track', 'Confirmed(Measurement) Track', 'Location', 'best');
            hold off;
        end
