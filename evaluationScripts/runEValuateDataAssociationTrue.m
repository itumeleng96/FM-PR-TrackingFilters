...
for pfa_index = 1:length(pfa_values)
    ...
    totalConfirmedKalman = 0;
    totalConfirmedUKF = 0;
    totalConfirmedParticle = 0;
    totalConfirmedRGNF = 0;
    ...
    for j = 1:numSimulations
        ...
        totalConfirmedKalman = totalConfirmedKalman + compareTracks(multiTargetTracker1.getConfirmedTracks, {trueData, trueData1,trueData2}, 0.5, 1, 10, 0.1);
        totalConfirmedUKF = totalConfirmedUKF + compareTracks(multiTargetTracker2.getConfirmedTracks, {trueData, trueData1,trueData2}, 0.5, 1, 10, 0.1);
        totalConfirmedParticle = totalConfirmedParticle + compareTracks(multiTargetTracker3.getConfirmedTracks, {trueData, trueData1,trueData2}, 0.5, 1, 10, 0.1);
        totalConfirmedRGNF = totalConfirmedRGNF + compareTracks(multiTargetTracker4.getConfirmedTracks, {trueData, trueData1,trueData2}, 0.5, 1, 10, 0.1);
    end

    % Calculate averages for the current PFA value
    avgKalmanResults(pfa_index) = totalConfirmedKalman / numSimulations;
    avgUKFResults(pfa_index) = totalConfirmedUKF / numSimulations;
    avgParticleResults(pfa_index) = totalConfirmedParticle / numSimulations;
    avgRGNFResults(pfa_index) = totalConfirmedRGNF / numSimulations;
end
...
function numConfirmedTrueTracks = compareTracks(confirmedTracks, trueTracks, rangeWeight, dopplerWeight, timeWindow, costOfNonAssignment)
    % Inputs:
    ...
    numConfirmedTrueTracks = size(assignments, 1);
end
