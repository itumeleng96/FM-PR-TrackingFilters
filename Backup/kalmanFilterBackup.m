classdef kalmanFilter
    ...
    properties
        dt,U,X,F,A,H,Q,R,P,S,coeff,measured_x,measured_y,std_acc,wk,innovations;
    end
    ...
    methods
        ...
        function [X_est,KF_obj2] = update(obj,z)
            ...
            obj.innovations = [obj.innovations,innovation];
            m=4;
            
            obj.R(2,2)=0.1;
            
            if(size(obj.innovations,2)>m)   
                %Only store last M items
                obj.innovations = obj.innovations(:,end-m+1:end);
                
                MIE1= mean(obj.innovations(1,:).^2);
                MIE2= mean(obj.innovations(2,:).^2);
                
                %calculate adaptive factor
                ak1 = (MIE1-obj.P(1,1))/obj.R(1,1);
                ak2 = (MIE2-obj.P(3,3))/obj.R(2,2);

                if(abs(ak2)>10)
                    obj.R(2,2) = abs(ak2)*obj.R(2,2);
                    %remove the abnormal measurement
            ...
            Sinv_doppler = inv(Sadapt_doppler);
            Sinv_range = inv(Sadapt_range);

            Ydoppler = Sinv_doppler * [obj.X(3:4);z(2)];
            Yrange = Sinv_range * [obj.X(1:2);z(1)];
                        
            Xadapt_range = Sinv_range * [eye(2);[1,0;]];
            Xadapt_doppler = Sinv_doppler * [eye(2); [1,0;]];
